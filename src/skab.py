...
def extract_data(data, augment_data=False):
    ...
    for task_index, task_data in enumerate(data):
        for instance, is_anom in task_data:
            instance_w_channels=np.expand_dims(np.array(instance), axis=2)
            #instance_w_height = np.expand_dims(instance, axis=0)
            samples.append(instance_w_channels)
